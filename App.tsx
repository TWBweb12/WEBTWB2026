...
function App() {
  ...
  // Promo popup auto-close countdown (10 seconds)
  useEffect(() => {
    ...
    const timer = setTimeout(() => setPopupCountdown((prev) => prev - 1), 1000);
    return () => clearTimeout(timer);
  }, [showPromoPopup, popupCountdown]);
  ...
  return (
    ...
    <div className="font-sans text-forest-dark bg-white overflow-x-hidden selection:bg-forest-green selection:text-white">
      ...
      {showPromoPopup && (
        ...
        >
          {/* Wrapper responsif — menyesuaikan perangkat */}
          <div
            style={{
              position: 'relative',
              /* Mobile: lebar penuh dengan padding; Tablet/Desktop: dibatasi proporsional */
              width: 'min(90vw, calc((100vh - 60px) * 0.75))',
              animation: 'popupFadeIn 0.45s cubic-bezier(0.34,1.56,0.64,1) both',
            }}
            onClick={(e) => e.stopPropagation()}
          >
            {/* Gambar popup */}
            {/* Gambar popup — klik buka WhatsApp */}
            <a
              href={`https://wa.me/628119102003?text=${encodeURIComponent('Halo, saya tertarik dengan promo September Escape di Taman Wisata Bougenville. Bisa dibantu untuk informasi dan reservasi?')}`}
              target="_blank"
              rel="noreferrer"
              onClick={() => { trackWhatsAppBooking('Promo Popup Banner'); setShowPromoPopup(false); }}
              style={{ display: 'block', cursor: 'pointer' }}
            >
              <img
                src="/Image/pop up 1.png"
                ...
                onMouseEnter={(e) => { (e.currentTarget as HTMLImageElement).style.opacity = '0.93'; }}
                onMouseLeave={(e) => { (e.currentTarget as HTMLImageElement).style.opacity = '1'; }}
              />
            </a>

            {/* Tombol X — di dalam foto, pojok kanan atas */}
            <button
            ...
            >
              ...
              <span style={{ color: '#c9a84c', fontWeight: 700 }}>{popupCountdown}</span> detik
            </div>
          </div>
